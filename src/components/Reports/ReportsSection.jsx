...
import OverdueByDept from './OverdueByDept';
import TasksByStatus from './TasksByStatus';
import TasksByDepartment from './TasksByDepartment';
import TasksByInterval from './TasksByInterval';
import TopOverdueComponents from './TopOverdueComponents';
import OverdueByCompetency from './OverdueByCompetency';
import TaskStatusTimeline from './TaskStatusTimeline';
import AvgOverdueByDepartment from './AvgOverdueByDepartment';
import TasksByVessel from './TasksByVessel';
...
const ReportsSection = ({ data }) => {
  ...
  return (
    <div className="mt-8 px-4">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">Reports Dashboard</h2>
        ...
        <div className="relative mb-4">
          ...
          >
            <div>
              <div className="font-semibold text-lg">{reports[currentReportIndex].title}</div>
              <div className="text-gray-600">{reports[currentReportIndex].description}</div>
            </div>
...
<div className="bg-white shadow-lg rounded-lg p-6 mb-6">
  ...
  <div>{reports[currentReportIndex].component}</div>
      ...
      <div className="text-center text-gray-600 mb-4">
        <p className="text-lg">
          Select a report from the menu above or use navigation buttons to move between reports
        </p>
      </div>
    </div>
  );
